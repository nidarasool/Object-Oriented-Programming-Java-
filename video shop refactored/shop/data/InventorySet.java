...
final class InventorySet implements Inventory {
  ...
  public Record get(Video v) {
	  /*Record ans = _data.get(v);
	  if (ans != null)
	  return ans;
	  else return null;*/
	 /*for (Video key : _data.keySet()) {
			if (key.equals(v)) { return _data.get(key); } }
		return null;*/
	  return _data.get(v);
  }
  ...
  Record checkOut(Video video) {
	  if (_data.containsValue(_data.get(video)) == false) 
		...
		if ((_data.get(video).numOut()) == (_data.get(video).numOwned())) {
			throw new IllegalArgumentException("numOut equals numOwned");
		}
		Record rec = _data.get(video);
		_data.put(video, new RecordObj(video, rec.numOwned(),_data.get(video).numOut()+1, _data.get(video).numRentals()+1));
		return rec;
  }
  ...
  Record checkIn(Video video) {
	  ...
	  Record rec = _data.get(video);
	  _data.put(video, new RecordObj(video, _data.get(video).numOwned(), _data.get(video).numOut()-1, _data.get(video).numRentals())); 
	 // if(rec.numOut() <0){ throw new IllegalArgumentException("Video has no record or numOut non-positive."); }
	 // _data.put(video, rec); 
	  return rec;
  }
  ...
  Map<Video,Record> clear() {
	  /*Map<Video, Record> datacop = new HashMap<Video, Record>(_data);
	  for(Video key : _data.keySet()) {
		  _data.remove(key);
	  }
    // TODO
    return datacop;*/
	  Map<Video,Record> cop = _data;
		_data = new HashMap<Video,Record>();
		return cop;
  }
